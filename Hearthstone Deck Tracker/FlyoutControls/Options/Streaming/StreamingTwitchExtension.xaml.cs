...
namespace Hearthstone_Deck_Tracker.FlyoutControls.Options.Streaming
{
	public partial class StreamingTwitchExtension : INotifyPropertyChanged
	{
		...
		public ICommand AuthenticateCommand => new Command(async () =>
		{
			...
			OAuthError = !success;
			if(success)
				RefreshTwitchAccountsCommand.Execute(null);
		});
		...
		public ICommand RefreshTwitchAccountsCommand => new Command(async () =>
		{
			await HSReplayNetOAuth.UpdateTwitchUsers();
			UpdateTwitchData();
		});

		public ICommand LinkTwitchAccountCommand => new Command(() => Helper.TryOpenUrl("https://hsreplay.net/account/social/connections/"));

		public ICommand InstallTwitchExtensionCommand => new Command(() => throw new NotImplementedException());
